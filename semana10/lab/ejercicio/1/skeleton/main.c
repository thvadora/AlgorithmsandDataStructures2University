...
void show_usage() {
    ...
    printf("-b Print values in decreasing order\n");
...
int run_file(const char *filename,
	...
	if (show_items) {
	    ...
	    //bool want_order = true; // change it to false if you dont want order
	    //while printing items
...
int main(int argc, char **argv) {
	...
	fails = run_file(filenames[i], do_backtracking,
			 do_dynamic, show_items, test, weight,order);
	printf("DONE %s.\n\n", filenames[i] == NULL ? "stdin" : filenames[i]);
    }
    if (fails > 0 && test) {
	printf("\n\n FAILS: %d \n\n", fails);
